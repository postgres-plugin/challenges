...
function register (server, options, next) {
  ...
  var getChalAndMatchingOrgs = function (chal_id, cb) {
    query(queries.getChalAndMatchingOrgs(chal_id), pool, function (queryErr, res) {
      if (queryErr) {
        return cb(queryErr);
      }

      return cb(null, formatters.getChalAndMatchingOrgs(res.rows));
    });
  };
  ...
  query(initQuery, pool, function (initErr) {
    ...
    server.method('pg.challenges.getChalAndMatchingOrgs', getChalAndMatchingOrgs);
