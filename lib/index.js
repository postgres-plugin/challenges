...
function register(server, options, next) {
  ...
  var getUsersArchived = function (userId, cb) {
    query(queries.getUsersArchived(userId), pool, function (queryErr, res) {
      return cb(queryErr, res.rows);
    });
  };

  var challengesSearch = function (searchTerm, tags, locations, cb) {
    query(queries.challengesSearch(searchTerm, tags, locations), pool, function (queryErr, res) {
      return cb(queryErr, res.rows);
    });
  };
  ...
  var addTags = function (itemId, tagIds, cb) {
    query(queries.addTags(itemId, tagIds), pool, function (queryErr, res) {

      return cb(queryErr, res.rows);
    });
  };

  var addLocations = function (itemId, tagIds, cb) {
    query(queries.addLocations(itemId, tagIds), pool, function (queryErr, res) {
  ...
  var getOwnerChallenge = function (challengeId, cb) {
    query(queries.getOwnerChallenge(challengeId), pool, function (queryErr, res) {

      return cb(queryErr, res.rows);
    });
  };

  var getUsersChallenges = function (userId, cb) {
    query(queries.getUsersChallenges(userId), pool, function (queryErr, res) {

      return cb(queryErr, res.rows);
    });
  };

  var challengeEditable = function (challengeId, userId, cb) {
    query(queries.challengeEditable(challengeId, userId), pool, function (queryErr, res) {

      return cb(queryErr, res.rows);
    });
  };

  var deleteChallenge = function (id, cb) {
    query(queries.deleteChallenge(id), pool, function (queryErr, res) {
      if (queryErr) {
        return cb(queryErr);
      }
      if (res.rowCount === 0) {
        return cb(Boom.notFound('Cannot delete a challenge with comments'));
      }
      return cb(queryErr, (res.rows));
    });
  };
  ...
  initQuery(options, pool, function (initErr, _) {
    if (initErr) {
      return next(initErr);
    }
    ...
    server.method('pg.challenges.getUsersArchived', getUsersArchived);
    ...
    server.method('pg.challenges.addTags', addTags);
    server.method('pg.challenges.addLocations', addLocations);
    server.method('pg.challenges.getOwnerChallenge', getOwnerChallenge);
    server.method('pg.challenges.getUsersChallenges', getUsersChallenges);
    server.method('pg.challenges.challengeEditable', challengeEditable);
    server.method('pg.challenges.deleteChallenge', deleteChallenge);
