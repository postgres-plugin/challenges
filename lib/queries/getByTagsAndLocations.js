...
module.exports = function (tagIds, locationsIds) {
  ...
  if(tagIds.length === 0 && locationsIds.length === 0) {
    return allChallenges.concat(['ORDER BY chal_date DESC, chal_title ASC']).join(' ');
  }

  if(tagIds.length > 0 && locationsIds.length > 0) {
    return tagQuery.concat(['UNION'], locationQuery, ['ORDER BY chal_date DESC, chal_title ASC']).join(' ')
  }

  if (tagIds.length > 0) {
    return tagQuery.concat(['ORDER BY chal_date DESC, chal_title ASC']).join(' ');
  }

  if(locationsIds.length >0) {
    return locationQuery.concat(['ORDER BY chal_date DESC, chal_title ASC']).join(' ');
  }
}
